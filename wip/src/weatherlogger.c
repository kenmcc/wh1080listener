...
FILE* create_or_open(void)
{
    ...
    strftime(buffer, 100, "mkdir -p raw/%Y/%Y-%m", timeinfo);
    system(buffer);
    
    strftime(buffer, 100, "raw/%Y/%Y-%m/%Y-%m-%d.txt", timeinfo);
...
//    printf("Total rain: %0.1f mm\n", rain);
    ...
    if(file != NULL)
    {
        ...
        fprintf(file,"1,50,20,%d,%0.1f,1000,%0.1f,%0.1f,%d,%0.1f\n",
              humidity, temperature, wind_avg_kmh, wind_gust_kmh, direction, rain);
        fclose(file);
    }
